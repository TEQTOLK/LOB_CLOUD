...
namespace LOB
{
    public partial class frmIPPRSettlement : Form
    {
        ...
        List<Tuple<int, string, string, double>> gPRNList = new List<Tuple<int, string, string, double>>();
        List<Tuple<int, string, string, double>> gPURList = new List<Tuple<int, string, string, double>>();
        private string gTranCode = "SUP005";
        private string gTranName = "SUPPLIER RETURN NOTE SETTLEMENT";
        ...
        private long UpdateOutstanding()
        {
            ...
            try
            {
                ...
                {
                    MySqlCommand cmd1 = new MySqlCommand("UPDATE tblPurchaseBrief SET TotalPaid = TotalPaid + " + Convert.ToDouble(dgvSettlement.Rows[i].Cells[7].Value) + " " +
                        " WHERE BillID = " + dgvSettlement.Rows[i].Cells[0].Value + "", GlobalData.GCon);
                    cmd1.ExecuteNonQuery();

                    MySqlCommand cmd2 = new MySqlCommand("UPDATE tblPRNBrief SET TotalPaid = TotalPaid + " + Convert.ToDouble(dgvSettlement.Rows[i].Cells[7].Value) + " " +
        ...
        private void SaveBillMatch()
        {
            ...
            {
                gSB.Clear();
                gSB.Append("INSERT INTO tblBillMatchSUP ( " +
                    " DateOn, PURBillID, PURBillNo, Outstanding, PRNBillID, PRNBillNo, PRNAmount, SettleAmount, " +
        ...
        private void MatchBill()
        {
            try
            {
                ...
                for (int i = 0; i < dgvSelectedBill.Rows.Count; i++)
                {
                    gPURList.Add(Tuple.Create(Convert.ToInt32(dgvSelectedBill.Rows[i].Cells[0].Value.ToString()),
                ...
                {
                    gPRNList.Add(Tuple.Create(Convert.ToInt32(dgvSelectedCCN.Rows[j].Cells[0].Value.ToString()),
                ...
                for (int i = 0; i < dgvSelectedBill.Rows.Count; i++)
                {
                    if (dgvSettlement.Rows.Count == 0)
                    {
                        if (gPURList[i].Item4 > gPRNList[i].Item4)
                        {
                            int k = 0;
                            ...
                            do
                            {
                                ...
                                {
                                    if (gBalAmount > gPRNList[k].Item4)
                                    {
                                        AddingGrid(gPURList[i].Item1, gPURList[i].Item2, gPURList[i].Item3, gPURList[i].Item4,
                                            gPRNList[k].Item1, gPRNList[k].Item3, gPRNList[k].Item4, gPRNList[k].Item4, 0);
                                        gBalAmount = gBalAmount - gPRNList[k].Item4;
                                        k++;
                                    }
                                    else
                                    {
                                        AddingGrid(gPURList[i].Item1, gPURList[i].Item2, gPURList[i].Item3, gPURList[i].Item4,
                                           gPRNList[k].Item1, gPRNList[k].Item3, gPRNList[k].Item4, gBalAmount, 0);
                                        gBalAmount = gBalAmount - gPRNList[k].Item4;
                                        k++;
                                        gBalAmount = 0;
                                    }
                        ...
                        {
                            int k = 0;
                            gBalAmount = Convert.ToDouble(gPRNList[i].Item4);
                            do
                            {
                                if (k != dgvSelectedCCN.Rows.Count)
                                {
                                    if (gBalAmount > gPURList[k].Item4)
                                    {
                                        AddingGrid(gPURList[i].Item1, gPURList[i].Item2, gPURList[i].Item3, gPURList[i].Item4,
                                            gPRNList[k].Item1, gPRNList[k].Item3, gPRNList[k].Item4, gPURList[k].Item4, 0);
                                        gBalAmount = gBalAmount - gPURList[k].Item4;
                                        k++;
                                    }
                                    else
                                    {
                    ...
                    else
                    {
                        ...
                        {
                            AddingGrid(gPURList[i].Item1, gPURList[i].Item2, gPURList[i].Item3, gPURList[i].Item4,
                               gPRNList[i].Item1, gPRNList[i].Item3, gPRNList[i].Item4, gPURList[i].Item4, 0);
                            gBalAmount = 0;
                        }
                    }
                }
