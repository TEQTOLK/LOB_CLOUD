...
namespace LOB
{
    public partial class frmIPBillCancel : Form
    {
        ...
        private long RoadToSave()
        {
            ...
            for (int i = 0; i < dgvMOP.Rows.Count; i++)
            {
                ...
                MySqlCommand cmd6 = new MySqlCommand("INSERT INTO tblDelSaleBrief ( " +
                    " BillId, DateOn, BillNo, CustID, Add1, Add2, City, Tp1, Tp2, Tp3, DescriptionOf, GrossTotal, " +
                    " NetTotal, CNAmount, TotalPaid, ManAmount, NetAmount, CashTendered, Balance, PerformedOn, UserID, " +
                    " UserName, CompID, CompName, IsDeleted, DelUserID, DelUserName, DelCompID, DelCompName, IsActive, " +
                    " DisRate, DisAmount, VatRate, VatAmount, StockRoomID, SystemRefNo, DueDate, CSHPaidOn, CHQPaidOn, " +
                    " CCSPaidOn, CREPaidOn, MOP, TranCode, NameOf, AdvanceAmount, DueAmount, RefundedAmount, IsPaused ) " +
                    " SELECT BillId, DateOn, BillNo, CustID, Add1, Add2, City, Tp1, Tp2, Tp3, DescriptionOf, GrossTotal, " +
                    " NetTotal, CNAmount, TotalPaid, ManAmount, NetAmount, CashTendered, Balance, PerformedOn, UserID, " +
                    " UserName, CompID, CompName, IsDeleted, " + GlobalData.GUserID + " AS DelUserID, '" + GlobalData.GUsername + "' AS DelUserName, " + GlobalData.GCompID + " AS DelCompID, '" + GlobalData.GCompName + "' AS DelCompName, IsActive, " +
                    " DisRate, DisAmount, VatRate, VatAmount, StockRoomID, SystemRefNo, DueDate, CSHPaidOn, CHQPaidOn, " +
                    " CCSPaidOn, CREPaidOn, MOP, TranCode, NameOf, AdvanceAmount, DueAmount, RefundedAmount, IsPaused " +
                    " FROM tblSaleBrief WHERE BillID = " + dgvMOP.Rows[i].Cells[7].Value + " ", GlobalData.GCon);
                cmd6.ExecuteNonQuery();

                MySqlCommand cmd3 = new MySqlCommand("DELETE FROM tblSaleBrief WHERE BillID = " + dgvMOP.Rows[i].Cells[7].Value + " ", GlobalData.GCon);
