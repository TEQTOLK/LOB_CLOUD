...
namespace LOB
{
    public partial class frmIPSUPPayment : Form
    {
        ...
        private string gTranCode = "SUP004";
        private string gTranName = "SUPPLIER PAYMENT";
        ...
        private void LoadOutstandingSelected(string cBillIDs)
        {
            ...
            dSet = GlbData.getDataSet(gSB.ToString(), "tblCashNotes");
            DataView dView = new DataView(dSet.Tables[0]);
            ...
            if (f.gOkCancel == true)
            {
                if (fConfirm.SayMessage("SV") == true)
                {
                    ...
                    else
                    {
                        ...
                        {
                            print.RoadToPrintReport(getCorrectQuery(), @"C:\LOB\Bin\Debug\Rep\SUPReceipt.rpt");
                        }
                        ClearLocal();
        ...
        private long RoadToSave(string cMOP, double cCNAmount, double cTotalPaid, decimal cCashTendered, DataGridView cDataGridView = null)
        {
            ...
            UpdateDealerOutstanding(cCashTendered);

            return gBriefID;
        }

        public void UpdateDealerOutstanding(decimal cTrenderedAmt)
        {
            double tmpCounter = 0;
            ...
            for (int i = 0; i < dgvBills.Rows.Count; i++)
            {
                ...
                if (tmpCounter > 0)
                {
                    ...
                    {
                        MySqlCommand cmd = new MySqlCommand("UPDATE tblPurchaseBrief SET TotalPaid = TotalPaid + " + tmpLineAmount + " WHERE BillID = " + tmpBillID + " AND CompID = " + GlobalData.GCompID + " ", GlobalData.GCon);
                        cmd.ExecuteNonQuery();
                        tmpCounter -= tmpLineAmount;
                    }
                    else
                    {
                        MySqlCommand cmd = new MySqlCommand("UPDATE tblPurchaseBrief SET TotalPaid = TotalPaid + " + tmpCounter + " WHERE BillID = " + tmpBillID + " AND CompID = " + GlobalData.GCompID + " ", GlobalData.GCon);
                        cmd.ExecuteNonQuery();
                        tmpCounter = 0;
                    }
                }
        ...
        private long SaveMOP(DataGridView cGridView, string cMOP)
        {
            ...
            if (cMOP == "CHQ")
            {
                for (int i = 0; i < cGridView.Rows.Count; i++)
                {
                    ...
                    tran.CompID = GlobalData.GCompID;
                    tran.ComputerName = GlobalData.GCompName;
                    tran.DelUserID = GlobalData.GUserID;
                    tran.DelUserName = GlobalData.GUsername;
                    tran.DelCompID = GlobalData.GCompID;
                    tran.DelComputerName = GlobalData.GCompName;
                    tran.BillID = 0;
                    tran.BillsFor = gBillIDs;
        ...
        {
            gBillNo = GlbData.getUniqueNumber("SUPPPAY", GlobalData.GCompID).ToString();
