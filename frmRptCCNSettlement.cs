...
namespace LOB
{
    public partial class frmRptCCNSettlement : Form
    {
        ...
        {
            return "SELECT  BR.BillID AS Qty, BR.DateOn AS BRDateOn, BR.BillNo AS BRBillNo, BR.NameOf AS CUName, BM.Outstanding AS TotalAmount, BM.CCNBillNo AS CUCode, " +
                " BM.CCNAmount AS Discount, BM.SettleAmount AS BRPaidAmount, BM.UserName AS CUCity, BM.PerformedOn AS E1IssuedDate, " +
                " BR.CompID AS UnitPrice, CO.Add1 + ' - ' + CO.City  AS Name " +
                " FROM tblBillMatchCUS AS BM INNER JOIN " +
                " tblSaleBrief AS BR ON BM.SaleBillID = BR.BillId INNER JOIN " +
        ...
        private void btnShow_Click(object sender, EventArgs e)
        {
            ...
            print.RoadToPrintReport(tmpQuery, @"C:\LOB\Bin\Debug\Rep\CCNSettlement.rpt", rptView, "P", gCSHTotal, gCHQTotal, gCCSTotal, gCRETotal);
