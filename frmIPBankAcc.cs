...
namespace LOB
{
    public partial class frmIPBankAcc : Form
    {
        ...
        private long SaveOrUpdateBank()
        {
            ...
            if (gEditFlag == false)
            {
                masterFiles.SaveBank();
                tmpID = 1;
            }
               
            else
                tmpID = masterFiles.UpdateBank();
        ...
        {
            MySqlCommand cmd = new MySqlCommand("DELETE FROM tblBankAccount WHERE Code= '" + txtCode.Text + "' ", GlobalData.GCon);
            cmd.ExecuteNonQuery();
            return 1;
        }
        ...
        {
            masterFiles.FillBank(txtCode.Text.Trim());
        ...
        private void txtCode_Validating(object sender, CancelEventArgs e)
        {
            ...
            gSB.Clear();
            gSB.Append("SELECT Code FROM tblBankAccount WHERE Code = '" + txtCode.Text.Trim() + "' ");
