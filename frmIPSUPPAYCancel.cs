...
namespace LOB
{
    public partial class frmIPSUPPAYCancel : Form
    {
        ...
        private void DisplayBills()
        {
            ...
            gSB.Append(" SELECT BR.DateOn, BR.BillNo, BR.NameOF, BR.City, CD.Amount AS CashTendered, BR.BillID " +
                ...
                " tblPurchaseBrief AS BR ON BR.BillId = CD.SaleBillId " +
                " WHERE CP.BillNo =  '" + txtRefNo.Text.Trim() + "' ");
            dataTable = GlbData.getDataTable(gSB.ToString());
        ...
        private void txtRefNo_Validating(object sender, CancelEventArgs e)
        {
            ...
            gSB.Append(" SELECT * FROM tblCustPayBrief " +
                " WHERE BillNo =  '" + txtRefNo.Text.Trim() + "' AND CompId = " + GlobalData.GCompID + " AND TranCode = 'SUP004' ");
        ...
        private long RoadToSave()
        {
            ...
            {
                MySqlCommand cmd = new MySqlCommand("UPDATE tblPurchaseBrief SET TotalPaid = TotalPaid - " + Convert.ToDouble(dgvBills.Rows[k].Cells[4].Value) + ", " +
