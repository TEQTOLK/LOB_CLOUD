...
namespace LOB
{
    public partial class frmIPSTKAdjustment : Form
    {
        ...
        private void txtQty_TextChanged(object sender, EventArgs e)
        {
            ...
            decimal tmpCurQty = 0;
            decimal.TryParse(txtCurQty.Text, out tmpCurQty);
        ...
        private void AddingGrid(int cItemID, string cCode, string cItemName, double cUnitPrice, double cQty, double cTotalAmount, double cCost, string cState, decimal cADQty)
        {
            ...
            newRow.Cells[7].Value = cState;
            newRow.Cells[8].Value = cADQty.ToString();
        ...
        private void txtQty_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                {
                    gADDDEDQty = 0;
                    decimal tmpCurQty, tmpQty = 0;
                    decimal.TryParse(txtCurQty.Text, out tmpCurQty);
                    decimal.TryParse(txtQty.Text, out tmpQty);

                    if (tmpQty > tmpCurQty)
                        gADDDEDQty = tmpQty - tmpCurQty;
                    else
                        gADDDEDQty = tmpCurQty - tmpQty;

                    AddingGrid(gCustID, txtCode.Text.Trim(), lblProduct.Text, Convert.ToDouble(txtPrice.Text), Convert.ToDouble(txtQty.Text), Convert.ToDouble(txtTotalAmount.Text), gCostPrice, cboStatus.Text, gADDDEDQty);
        ...
        private void SaveStockFlow()
        {
            ...
            tran.SaveSTOCKFlow(gBriefID, dgvOpenStock);
        }
        ...
        private void UpdateItemStock()
        {
            for (int i = 0; i < dgvOpenStock.Rows.Count; i++)
            {
                if(dgvOpenStock.Rows[i].Cells[7].Value.ToString() == "ADD")
                    tran.SaveOrUpdateStock(Convert.ToInt32(dgvOpenStock.Rows[i].Cells[0].Value), "A", Convert.ToDouble(dgvOpenStock.Rows[i].Cells[8].Value));
                else
                    tran.SaveOrUpdateStock(Convert.ToInt32(dgvOpenStock.Rows[i].Cells[0].Value), "D", Convert.ToDouble(dgvOpenStock.Rows[i].Cells[8].Value));
            }
        }
        ...
        private long RoadToSave()
        {
            ...
            if (gBriefID > 0)
            {
                tran.SaveSTKADJDetail(gBriefID, dgvOpenStock);
                SaveStockFlow();
                UpdateItemStock();
            }
            return gBriefID;
        ...
        private bool IsRequired()
        {
            ...
            gSB.Clear();
            gSB.Append("SELECT * FROM tblSTKADJBrief WHERE BillNo = '" + txtRefNo.Text.Trim() + "' ");
            if (GlbData.AlreadyExist(gSB.ToString()))
            {
