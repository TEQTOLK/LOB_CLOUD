...
namespace LOB
{
    public partial class frmIPPO : Form
    {
        private double gCostPrice = 0;
        private int gItemID = 0;
        private string gTranCode = "SUP008";
        private string gStockRoomName = "";
        private long gBriefID = 0;
        private string gTranName = "PURCHASE ORDER";
        ...
        {
            frmIPItemSearch f = new frmIPItemSearch();
            f.ShowDialog();
        ...
        {
            string tmpBillNo = GlbData.getUniqueNumber("PO", GlobalData.GCompID).ToString();
            long tmpID = 0;
            ...
            tran.PurchaseOrderNo = txtPOOrderNo.Text;
            tran.CSHPaidOn = 0;
            tran.CCSPaidOn = 0;
            tran.CHQPaidOn = 0;
            tran.CREPaidOn = 0;

            tmpID = tran.SavePOBrief();
        ...
        private void SaveStockFlow()
        {
            ...
            tran.SaveSTOCKFlow(gBriefID, dgvGrid, "A");
        }
        ...
        private void UpdateItemStock()
        {
            for (int i = 0; i < dgvGrid.Rows.Count; i++)
            {
                ...
                tran.SaveOrUpdateStock(Convert.ToInt32(dgvGrid.Rows[i].Cells[0].Value), "A", Convert.ToDouble(dgvGrid.Rows[i].Cells[5].Value));

                if (Convert.ToDouble(dgvGrid.Rows[i].Cells[4].Value) > 0)
                    tran.SaveOrUpdateStock(Convert.ToInt32(dgvGrid.Rows[i].Cells[0].Value), "A", Convert.ToDouble(dgvGrid.Rows[i].Cells[4].Value));
        ...
        private long RoadToSave()
        {
            ...
            if (gBriefID > 0)
            {
                tran.SavePODetail(gBriefID, dgvGrid);
                //SaveStockFlow();
                //UpdateItemStock();
            }
            return gBriefID;
        ...
        private bool IsRequired()
        {
            ...
            if(txtGRNAmount.Text == "")
            {
                MessageBox.Show("Please enter GRN Value", "INVALID", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                txtGRNAmount.Focus();
                return false;
            }
            gSB.Clear();
            gSB.Append("SELECT * FROM tblPOBrief WHERE BillNo = '" + txtRefNo.Text.Trim() + "' ");
            if (GlbData.AlreadyExist(gSB.ToString()))
            {
            ...
            //if(Convert.ToDouble(lblNetTotal.Text) != Convert.ToDouble(txtGRNAmount.Text))
            //{
            //    MessageBox.Show("Sorry.. Please enter a correct Purchase Value", "INVALID", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            //    txtGRNAmount.Focus();
            //    return false;
            //}
            return true;
        ...
        private void txtPOOrderNo_Validating(object sender, CancelEventArgs e)
        {
            
        }

        private void txtGRNAmount_Validating(object sender, CancelEventArgs e)
        {
            txtGRNAmount.Text = string.Format("{0:N}", Convert.ToDouble(txtGRNAmount.Text));
        }

        private void frmIPPurchase_Load(object sender, EventArgs e)
        {
        ...
        {
            frmIPPOAnalizer f = new frmIPPOAnalizer();
            f.ShowDialog();

            if(f.gOkCancel == true)
            {
                for (int i = 0; i < f.dgvPO.Rows.Count; i++)
                {
                    AddingGrid(
                        Convert.ToInt32(f.dgvPO.Rows[i].Cells[0].Value),
                        f.dgvPO.Rows[i].Cells[1].Value.ToString(),
                        f.dgvPO.Rows[i].Cells[2].Value.ToString(),
                        Convert.ToDouble(f.dgvPO.Rows[i].Cells[3].Value),
                        Convert.ToDouble(f.dgvPO.Rows[i].Cells[4].Value),
                        Convert.ToDouble(f.dgvPO.Rows[i].Cells[5].Value),
                        Convert.ToDouble(f.dgvPO.Rows[i].Cells[6].Value),
                        Convert.ToDouble(f.dgvPO.Rows[i].Cells[7].Value));
                    lblNetTotal.Text = string.Format("{0:N}", CalcTotalAmount());
                    lblSubTotal.Text = string.Format("{0:N}", CalcTotalAmount());
                    lblTotItems.Text = dgvGrid.Rows.Count.ToString();
                }
