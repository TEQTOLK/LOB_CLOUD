...
namespace LOB
{
    public partial class frmIPCCN : Form
    {
        private double gCostPrice = 0;
        private int gItemID = 0;
        private string gTranCode = "CUS003";
        private string gStockRoomName = "";
        private long gBriefID = 0;
        private string gTranName = "SALES RETURN";
        ...
        private bool IsRequired()
        {
            ...
            gSB.Clear();
            gSB.Append("SELECT * FROM tblCCNBrief WHERE BillNo = '" + txtRefNo.Text.Trim() + "' ");
            if (GlbData.AlreadyExist(gSB.ToString()))
            {
        ...
        private long RoadToSave()
        {
            ...
            if (gBriefID > 0)
            {
                tran.SaveCCNDetail(gBriefID, dgvGrid);
                SaveStockFlow();
                UpdateItemStock();
            }
            return gBriefID;
        ...
        {
            string tmpBillNo = GlbData.getUniqueNumber("CCN", GlobalData.GCompID).ToString();
            long tmpID = 0;
            ...
            tran.VatAmount = 0;
            tran.StockRoomID = 0;
            tran.SystemRefNo = txtRefNo.Text.Trim();
            ...
            tran.DelComputerName = "";
            tran.IsActive = "Y";
            tran.TranCode = "CUS003";
            tran.NameOf = lblSupplier.Text;

            tmpID = tran.SaveCCNBrief();
