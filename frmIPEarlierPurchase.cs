...
namespace LOB
{
    public partial class frmIPEarlierPurchase : Form
    {
        private double gCostPrice = 0;
        private int gItemID = 0;
        private string gTranCode = "SUP007";
        private string gStockRoomName = "";
        private long gBriefID = 0;
        private string gTranName = "EARLIER PURCHASE";
        ...
        private bool IsRequired()
        {
            ...
            gSB.Clear();
            gSB.Append("SELECT * FROM tblPurchaseBrief WHERE BillNo = 'EP-' + '" + txtINVNo.Text.Trim() + "' ");
            if (GlbData.AlreadyExist(gSB.ToString()))
            {
                MessageBox.Show("Sorry.. This GRN NO alredy exist", "INVALID", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                txtINVNo.Focus();
                return false;
            }
            return true;
        ...
        private long SaveBrief()
        {
            ...
            for (int i = 0; i < dgvGrid.Rows.Count; i++)
            {
                ...
                tran.BillNo = "EP-" + dgvGrid.Rows[i].Cells[4].Value.ToString();
                tran.MOP = "CRE";
                tran.CustID = gCustID;
                tran.Add1 = gAdd1;
                tran.Add2 = gAdd2;
        ...
        {
            gSB.Clear();
            gSB.Append("SELECT * FROM tblItem WHERE Code = 'EP'");

            if(!GlbData.AlreadyExist(gSB.ToString()))
            {
                masterFiles.Code = "EP";
                masterFiles.NameOF = "EARLIER PURCHASE";
