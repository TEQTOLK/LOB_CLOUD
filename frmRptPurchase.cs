    ...
    public partial class frmRptPurchase : Form
    {
        ...
        private void btnShow_Click(object sender, EventArgs e)
        {
            ...
            if (cboBasedOn.SelectedIndex == 2)
            {
                ...
                gCSHTotal = Convert.ToDouble(GlbData.GetValue("SELECT DisAmount FROM tblPurchaseBrief WHERE BillNo = '" + txtInvoice.Text.Trim() + "'", "DisAmount"));
                print.RoadToPrintReport(tmpQuery, @"C:\LOB\Bin\Debug\Rep\GRN.rpt", rptView, "P", gCSHTotal, gCHQTotal, gCCSTotal, gCRETotal);
            }
               
            else
            {
                tmpQuery = GetQuery() + getDateRange() + getCustomerID() + " ORDER BY BR.DateOn" ;
                print.RoadToPrintReport(tmpQuery, @"C:\LOB\Bin\Debug\Rep\Purchase.rpt", rptView, "P", gCSHTotal, gCHQTotal, gCCSTotal, gCRETotal);
            }
        ...
        private string getPaticularInvoice()
        {
            return " SELECT BR.BillNo AS BRBillNo, BR.DateOn AS BRDateOn, BR.UserName AS E1Name, " +
                ...
                " FROM tblPurchaseBrief AS BR INNER JOIN " +
                " tblPurchaseDetail AS DT ON BR.BillID = DT.BriefID INNER JOIN " +
