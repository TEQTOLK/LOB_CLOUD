...
namespace LOB
{
    public partial class frmIPGoodTransferNote : Form
    {
        public int gCustID = 0;
        private DateTime gServerDate = DateTime.Now;
        private string gTranCode = "STO004";
        private string gStockRoomName = "";
        private long gBriefID = 0;
        private double gCostPrice = 0;
        private string gTranName = "STOCK TRANSFER";
        private double gCurQty = 0;
        private int gFromRoomID = 0;
        private int gToRoomID = 0;
        ...
        private void fillProduct()
        {
            ...
            txtCurQty.Text = gCurQty.ToString();
        }
        ...
        private Boolean IsRequiredGrid()
        {
            ...
            if(cboFrom.Text == "")
            {
                MessageBox.Show("Empty From StockRoom", "INVALID", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                cboFrom.Focus();
                return false;
            }
            if(cboTo.Text == "")
            {
                MessageBox.Show("Empty To StockRoom", "INVALID", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                cboTo.Focus();
                return false;
            }
            if(cboFrom.Text == cboTo.Text)
            {
                MessageBox.Show("Both StockRooms are cannot be same", "INVALID", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                cboFrom.Focus();
                return false;
            }
            return true;
        ...
        private long SaveBrief()
        {
            ...
            tran.FromStockroom = gFromRoomID;
            tran.ToStockroom = gToRoomID;

            tmpID = tran.SaveGTNBrief();

            return tmpID;
        }

        private void FillFromRoom()
        {
            string cQuery = "SELECT * FROM tblStockRoom WHERE IsActive = 'Y' ORDER BY NameOF";
            cboFrom.DataSource = GlbData.getDataSet(cQuery, "tblStockRoom").Tables[0];
            cboFrom.ValueMember = "CustID";
            cboFrom.DisplayMember = "NameOF";
        }

        private void FillToRoom()
        {
            string cQuery = "SELECT * FROM tblStockRoom WHERE IsActive = 'Y' ORDER BY NameOF";
            cboTo.DataSource = GlbData.getDataSet(cQuery, "tblStockRoom").Tables[0];
            cboTo.ValueMember = "CustID";
            cboTo.DisplayMember = "NameOF";
        }

        private void frmIPGoodTransferNote_Load(object sender, EventArgs e)
        {
            FillFromRoom();
            FillToRoom();
        }

        private void cboFrom_Validating(object sender, CancelEventArgs e)
        {
            if (cboFrom.Text == "")
                return;
            else
                gFromRoomID = Convert.ToInt32(cboFrom.SelectedValue);
        }

        private void cboTo_Validating(object sender, CancelEventArgs e)
        {
            if (cboTo.Text == "")
                return;
            else
                gToRoomID = Convert.ToInt32(cboTo.SelectedValue);
        }
        ...
        private void SaveStockFlow(int cStockRoomID, string cAD)
        {
            tran.DateOn = gServerDate;
            ...
            tran.TranCode = gTranCode;
            tran.StockRoomID = cStockRoomID;

            tran.SaveSTOCKFlow(gBriefID, dgvOpenStock, cAD);
        }
        private bool IsRequired()
        {
            ...
            gSB.Clear();
            gSB.Append("SELECT * FROM tblGTNBrief WHERE BillNo = '" + txtRefNo.Text.Trim() + "' ");
            if (GlbData.AlreadyExist(gSB.ToString()))
            {
        ...
        private void UpdateItemStock(int cStockRoomID, string cAD)
        {
            for (int i = 0; i < dgvOpenStock.Rows.Count; i++)
            {
                tran.SaveOrUpdateStock(Convert.ToInt32(dgvOpenStock.Rows[i].Cells[0].Value), cAD, Convert.ToDouble(dgvOpenStock.Rows[i].Cells[4].Value), cStockRoomID);
            }
        }
        ...
        private long RoadToSave()
        {
            ...
            if (gBriefID > 0)
            {
                tran.SaveGTNDetail(gBriefID, dgvOpenStock);
                SaveStockFlow(gFromRoomID, "D");
                SaveStockFlow(gToRoomID, "A");
                UpdateItemStock(gFromRoomID, "D");
                UpdateItemStock(gToRoomID, "A");
            }
            return gBriefID;
        }
