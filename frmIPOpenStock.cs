...
namespace LOB
{
    public partial class frmIPOpenStock : Form
    {
        ...
        private void SaveStockFlow()
        {
            ...
            tran.SaveSTOCKFlow(gBriefID, dgvOpenStock, "A");
        }
        ...
        private void UpdateItemStock()
        {
            for (int i = 0; i < dgvOpenStock.Rows.Count; i++)
            {
                ...
                tran.SaveOrUpdateStock(Convert.ToInt32(dgvOpenStock.Rows[i].Cells[0].Value), "A", Convert.ToDouble(dgvOpenStock.Rows[i].Cells[4].Value));
        ...
        private long RoadToSave()
        {
            ...
            if(gBriefID > 0)
            {
                tran.SaveOpenMainDetail(gBriefID, dgvOpenStock);
                SaveStockFlow();
                UpdateItemStock();
            }
            return gBriefID;
        ...
        private bool IsRequired()
        {
            ...
            gSB.Clear();
            gSB.Append("SELECT * FROM tblOpenMainBrief WHERE BillNo = '" + txtRefNo.Text.Trim() + "' ");
            if(GlbData.AlreadyExist(gSB.ToString()))
            {
