...
namespace LOB
{
    public partial class frmIPPurchase : Form
    {
        private double gCostPrice = 0;
        private int gItemID = 0;
        private string gTranCode = "SUP002";
        private string gStockRoomName = "";
        private long gBriefID = 0;
        private string gTranName = "PURCHASE";
        ...
        {
            string tmpBillNo = GlbData.getUniqueNumber("PURCHASE", GlobalData.GCompID).ToString();
            long tmpID = 0;
        ...
        private long RoadToSave()
        {
            ...
            if (gBriefID > 0)
            {
                tran.SavePurchaseDetail(gBriefID, dgvGrid);
                SaveStockFlow();
                UpdateItemStock();
            }
            return gBriefID;
        ...
        private bool IsRequired()
        {
            ...
            gSB.Clear();
            gSB.Append("SELECT * FROM tblPurchaseBrief WHERE BillNo = '" + txtRefNo.Text.Trim() + "' ");
            if (GlbData.AlreadyExist(gSB.ToString()))
            {
