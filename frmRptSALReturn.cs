...
namespace LOB
{
    public partial class frmRptSALReturn : Form
    {
        ...
        private void btnShow_Click(object sender, EventArgs e)
        {
            ...
            if (cboBasedOn.SelectedIndex == 2)
            {
                ...
                gCSHTotal = Convert.ToDouble(GlbData.GetValue("SELECT DisAmount FROM tblCCNBrief WHERE BillNo = '" + txtInvoice.Text.Trim() + "'", "DisAmount"));
                print.RoadToPrintReport(tmpQuery, @"C:\LOB\Bin\Debug\Rep\SALReturn.rpt", rptView, "P", gCSHTotal, gCHQTotal, gCCSTotal, gCRETotal);
            }
        ...
        private string getPaticularInvoice()
        {
            return " SELECT BR.BillNo AS BRBillNo, BR.DateOn AS BRDateOn, BR.UserName AS E1Name, " +
                ...
                " FROM tblCCNBrief AS BR INNER JOIN " +
                " tblCCNDetail AS DT ON BR.BillID = DT.BriefID INNER JOIN " +
