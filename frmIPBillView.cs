...
namespace LOB
{
    public partial class frmIPBillView : Form
    {
        ...
        {
            return "SELECT BillID, CAST(DateOn AS Date) AS DateOn, BillNo, NameOf + '-' + City AS NameOF, ManAmount, PerformedOn " +
                " FROM " + TABLE_NAME + " WHERE IsActive = 'Y' ";
        }

        private string getDateRange()
        {
            return " AND DateOn BETWEEN CONVERT(date, '" + dtpFrom.Text + "') AND CONVERT(date, '" + dtpTo.Text + "')";
        }

        private void fillBillView()
        {
            string strQuery = correctQuery() + getDateRange() + "ORDER BY DateOn, BillNo";
            dSet = GlbData.getDataSet(strQuery, TABLE_NAME);
